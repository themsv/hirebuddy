...
const ListOfInterviews = ({ candidateDetails, userDetails }) => {
  ...
  const handleOutcomeFilter = (value) => {
    setRows(originalData.filter((item) => item.outcome === value));
  };

  const handleCareerAppliedFilter = (value) => {
    setRows(
      originalData.filter(
        (item) => item.candidateCareerStageInterviewedFor === value
      )
    );
  };

  const handleCareerSelectedFilter = (value) => {
    setRows(
      originalData.filter((item) => item.recommendedCareerStage === value)
    );
  };
      ...
      <TableContainer sx={{ maxHeight: 440 }}>
        ...
        >
          <Header
            ...
            onRequestOutcome={handleOutcomeFilter}
            onRequestCareerApplied={handleCareerAppliedFilter}
            onRequestCareerSelected={handleCareerSelectedFilter}
          />
